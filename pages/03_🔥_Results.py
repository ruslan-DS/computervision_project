...
st.write("""
  ...
  Изначальные проблемы с исхдной выборкой и большая вероятность сгенерировать не репрезентативную выборку сильно повлияли на модель.
  Использование всей выборки на ещё меньшей модели (например, YOLOv5s) привели бы к лучшим результатам.
""")
...
st.write("""
 ...
 \n Модель обучалась около 4 часов, так и не заверила до конца процесс обучения до 150 эпох. В целом, результат устраивает, видно положительное влияние всей выборки.
 ...
 Модель была запущена вчера днем. Сегодня было дообучение ещё на 50 эпохах. Пока функцианирует модель со вчерашними весами.
""")
...
st.write("""
   ...
   \n Пришлось создать около 6 дополнительных почт для Google Colab. Использование двух ноутбуков, и много часов ожидания, лишь бы не упал Colab.
   Однако, за это время с данными ресурсами удалось запустить 4 раза обучение на AutoEncoder с тестированием различной архитектуры модели. А также
   6 раз обучения различных моделей YOLOv5, начиная с YOLOv5m, заканчивая самой большой - YOLOv5x. Наилучший результат на данный момент показала самая
   маленькая модель, которая обучается на всей выборке. В будущем для предсказания фотографий на данной странцие будут использоваться именно эти веса для YOLOv5m.
   Всем спасибо! :)
""")